...
ManageFilesRepo::ManageFilesRepo(Database& db) : dbConn(db) {}

CorpusMetadata::CorpusFile 
ManageFilesRepo::uploadFileContent(const int group_id,
                                   ...
                                   const std::string& file_name)
{
    ...
    if (sqlite3_prepare_v2(dbConn.get(), fileSql, -1, &rawFileStmt, nullptr) != SQLITE_OK)
    ...
    int file_id = static_cast<int>(sqlite3_last_insert_rowid(dbConn.get()));
    if (file_id <= 0)
...
void ManageFilesRepo::insertFileText(const int group_id, const int file_id, const std::string& file_content)
{
...
void ManageFilesRepo::batchInsert(const int group_id,
                                    ...
                                    const std::string& col_name)
{
    ...
    if (sqlite3_prepare_v2(dbConn.get(), sql.c_str(), -1, &rawStmt, nullptr) != SQLITE_OK)
        throw std::runtime_error(sqlite3_errmsg(dbConn.get()));
...
CorpusMetadata::DeleteFileResult ManageFilesRepo::deleteAFile(int file_id)
{
    // One atomic domain operation
    TransactionGuard txn(dbConn.get());
...
CorpusMetadata::CorpusFileText ManageFilesRepo::getFileText(int file_id)
{
    sqlite3_stmt* rawStmt = nullptr;
    const char* sql =
