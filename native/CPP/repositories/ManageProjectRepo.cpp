...
ManageProjectRepo::ManageProjectRepo(Database& db) : dbConn(db) {}

CorpusMetadata::ProjectTitle ManageProjectRepo::startNewProject(const std::string& project_title)
{
    TransactionGuard tsx(dbConn.get());

    sqlite3_stmt* raw = nullptr;

    const char* sql = "INSERT INTO project (project_name) VALUES (?);";

    if (sqlite3_prepare_v2(dbConn.get(), sql, -1, &raw, nullptr) != SQLITE_OK)
        throw std::runtime_error(std::string("Prepare failed: ") + sqlite3_errmsg(dbConn.get()));
    
    StmtGuard stmt(raw);

    if (sqlite3_bind_text(stmt.get(), 1, project_title.c_str(), -1, SQLITE_TRANSIENT) != SQLITE_OK)
        throw std::runtime_error(std::string("Bind failed:") + sqlite3_errmsg(dbConn.get()));

    if (sqlite3_step(stmt.get()) != SQLITE_DONE)
        throw std::runtime_error(std::string("Insert failed: ") + sqlite3_errmsg(dbConn.get()));

    int project_id = static_cast<int>(sqlite3_last_insert_rowid(dbConn.get()));
...
CorpusMetadata::ProjectTitle ManageProjectRepo::updateProjectTitle(int project_id, const std::string& project_title)
{
    TransactionGuard txn(dbConn.get());

    sqlite3_stmt* rawStmt = nullptr;
    const char* sql =
        "UPDATE project SET project_name = ? WHERE id = ?;";

    if (sqlite3_prepare_v2(dbConn.get(), sql, -1, &rawStmt, nullptr) != SQLITE_OK)
        throw std::runtime_error(std::string("Prepare failed:") + sqlite3_errmsg(dbConn.get()));
...
std::vector<CorpusMetadata::ProjectTitle> ManageProjectRepo::getProjectTitles()
{
    ...
    while (true) {
        ...
        if (rc == SQLITE_ROW) {
            ...
            throw std::runtime_error(sqlite3_errmsg(dbConn.get()));
        }
    }
...
CorpusMetadata::CorpusMetadata ManageProjectRepo::getProjectMetadata(int project_id)
{
    ...
    if (sqlite3_prepare_v2(dbConn.get(), sql1, -1, &rawStmt1, nullptr) != SQLITE_OK)
        throw std::runtime_error(std::string("Prepare failed:") + sqlite3_errmsg(dbConn.get()));
    ...
    if (sqlite3_prepare_v2(dbConn.get(), sql2, -1, &rawStmt2, nullptr) != SQLITE_OK)
        throw std::runtime_error(sqlite3_errmsg(dbConn.get()));
