...
export const loadAllProjectTitles = async (): Promise<ProjectTitle[]> => {
    ...
    const raw = await window.api.loadAllProjectTitles();
    console.log(raw);
...
export const loadProjectMetadata = async (projectId: number): Promise<CorpusMetaData> => {
    ...
    try {
        const raw = await window.api.loadProjectMetadata(projectIdObject);

        const text = typeof raw === "string" ? raw : String(raw);
        const trimmed = text.trim();
        if (!trimmed) {
            throw new Error("Empty IPC response in loadProjectMetadata");
        }

        const corpusMetadata: CorpusMetaData = JSON.parse(trimmed);
...
export const postCorpusName = async (corpusDetails: CorpusProjectRelation): Promise<ApiResponse> => {
    try {
        const raw = await window.api.postCorpusName(corpusDetails);

        if (typeof raw === "string") {
            const trimmed = raw.trim();
            if (!trimmed) return { status: "fail", cppOutput: "Empty IPC response in postCorpusName" };
...
export const patchCorpusName = async (corpus: Corpus): Promise<ApiResponse> => {
    try {
        const raw = await window.api.patchCorpusName(corpus);

        if (typeof raw === "string") {
            const trimmed = raw.trim();
            if (!trimmed) return { status: "fail", cppOutput: "Empty IPC response in patchCorpusName" };
            return JSON.parse(trimmed) as ApiResponse;
        }
...
export const postGroupName = async (groupName: string, corpus: Corpus): Promise<ApiResponse> => {
    try {
        const raw = await window.api.postGroupName(groupName, corpus);

        if (typeof raw === "string") {
            const trimmed = raw.trim();
            if (!trimmed) return { status: "fail", cppOutput: "Empty IPC response in postGroupName" };
            return JSON.parse(trimmed) as ApiResponse;
        }
...
export const patchGroupName = async (groupName: string, group_id: number): Promise<ApiResponse> => {
    ...
    try {
        const raw = await window.api.patchGroupName(subCorpusData);

        if (typeof raw === "string") {
            const trimmed = raw.trim();
            if (!trimmed) return { status: "fail", cppOutput: "Empty IPC response in patchGroupName" };
            const parsed = JSON.parse(trimmed) as ApiResponse;
            console.log("In the patchGroupName function: ", parsed);
            return parsed;
        }

        console.log("In the patchGroupName function: ", raw);
        return raw as ApiResponse;
    } catch (error) {
        throw error;
    }
...
export const uploadFileContent = async (fileContent: string, group_id: number, file_name: string): Promise<ApiResponse> => {
    ...
    try {
        const raw = await window.api.uploadFileContent(corpusFileContent);

        if (typeof raw === "string") {
            const trimmed = raw.trim();
            if (!trimmed) return { status: "fail", cppOutput: "Empty IPC response in uploadFileContent" };
            return JSON.parse(trimmed) as ApiResponse;
        }
...
export const getCorpusFileText = async (file_id: number): Promise<ApiResponse> => {
    ...
    try {
        const raw = await window.api.getCorpusFileText(corpusFile);

        if (typeof raw === "string") {
            const trimmed = raw.trim();
            if (!trimmed) return { status: "fail", cppOutput: "Empty IPC response in getCorpusFileText" };
            return JSON.parse(trimmed) as ApiResponse;
        }
