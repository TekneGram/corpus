...
void DatabaseSummarizer::countWordsPerFile(const int& corpus_id)
{
    ...
    const char* sql = R"(
        SELECT
            ...
            words.file_id,
            COUNT(DISTINCT words.word) AS word_count,
    ...
    const char* insertSQL = R"(
        INSERT INTO words_counts_per_file (group_id, file_id, word_count, token_count) VALUES (?, ?, ?, ?);
    )";
    ...
    while (sqlite3_step(statement) == SQLITE_ROW) {
        ...
        int word_count = sqlite3_column_int(statement, 2);
        ...
        if (sqlite3_bind_int(insertStatement, 1, group_id) != SQLITE_OK ||
            ...
            sqlite3_bind_int(insertStatement, 3, word_count) != SQLITE_OK ||
